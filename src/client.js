...
const { generateSchedule, scheduleDispatches, dailyReset } = require("./scheduler");
const { queueDispatch } = require("./dispatcher");
const { extractNeighborhood, classifyNeighborhood, resolveGroups } = require("./neighborhood");
const { handleStatus, handleClear, handleGroups, handleFire } = require("./commands");
...
// Expõe o client via state para uso em dispatcher.js (evita dependência circular)
state.client = client;
...
client.on("ready", async () => {
  ...
  state.dispatchesDone = state.todayQueue.filter((m) => m.status === "sent").length;

  const savedSchedule = persistence.loadSchedule();
  if (savedSchedule) {
    state.SCHEDULE = savedSchedule;
    await persistence.log("BOT", `Schedule loaded from disk: ${state.SCHEDULE.join(", ")}`);
  } else {
    state.SCHEDULE = generateSchedule();
    persistence.saveSchedule();
    await persistence.log("BOT", `Schedule generated: ${state.SCHEDULE.join(", ")}`);
  }
...
client.on("message_create", async (msg) => {
  ...
  if (msg.body === "!status") { await handleStatus(msg); return; }
  if (msg.body === "!clear")  { await handleClear(msg);  return; }
  if (msg.body === "!groups") { await handleGroups(client, msg); return; }
  if (msg.body.startsWith("!fire")) { await handleFire(msg); return; }
